...
* Audience
  This doument is meant for System Administrators who want to setup the College Cloud in their institute, the two machine style. 
...
* Requirements 
  ...
  The following are a list of recommended requirements to setup the college cloud.
  ...
  1. Two machines with the following configuration
     ...
     + edX Machine :: This machine is dedicated solely for the edX platform to run.     
     #+BEGIN_EXAMPLE
     Processor : Intel(R) Core(TM) i7-3770 , CPU with 3.40GHz (Model : intel-db75en) 
     RAM : 16 GB 
     No of Cores : 8  
...
*** More on cluster machine
   ...
   :END:    
    There are other things in cluster machine. They are
...
*** Configuring edX College Cloud Server
   ...
   6. Finally, fire this commands from =vlabs= user
      #+BEGIN_SRC 
      cd vagrant-boxes
      vagrant up --provision
      #+END_SRC
      
   7. Set *no proxy* on browsers
   
   8. Access http://vlabs.ac.in page
   
   9. Choosing required labs :
