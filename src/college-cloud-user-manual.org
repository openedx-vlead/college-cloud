...
* Configuring College Cloud Server
  ...
  5. Please use [[Passwords][default linux user credentials]] to login.
...
**** System Administrator/Workshop coordinator 
     ...
     - Log in using admin user credentials. Admin user credentials are
       at [[Passwords][passwords]]
      + NOTE :: It is advised that the password is changed after the
                first login . For the steps for changing the password,
                follow the [[Open edX platform admin user password][link]].
...
** TODO Workshop conducted by Workshop Coordinator
   + [[https://vlabs.ac.in:5959/college-cloud-edition][Organising workshops]]
     ...
     +  TODO Make sure the [[Requirements][requirements]] are met by the college.
...
** TODO Virtual Labs College Cloud Edition used by colleges as part of their curriculum
...
*** On existing College Cloud Server
  - Log in to the existing *College Cloud Server (old)* as a super user
    using [[Passwords][default user credentials]]
    #+BEGIN_EXAMPLE
    su -
...
*** Setup new machine as College Cloud Server
    ...
    - Make sure that the machine meets the [[Requirements][requirements]]
    - [[Configuring College Cloud Server][Configure College Cloud Server]]
    - [[Using College Cloud Edition]]
    - [[Workflow of College Cloud Edition]]
    - [[COMMENT Analytics and Feedback]]
