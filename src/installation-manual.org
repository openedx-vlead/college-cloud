...
* Requirements
  ...
  + College Cloud System Configuration (recommended) 
    ...
    #+END_EXAMPLE
  + WiFi Router
  + Computer Lab with at least 30-50 Systems/Desktops.
...
* Configuring College Cloud Server
  ...
  5. Please use [[./installation-manual.org::#passwords][default linux user credentials]] to login.
     + NOTE :: It is advised that the password is changed after the
               first login . To change the password, follow the [[./installation-manual.org::#Passwords][]]
...
** Registration (first time only) 
   ...
   the first use For registration please follow the steps follwing
   steps -

   
   + Open a web browser on a machine connected to college-cloud/
     machine form which college-cloud was accessible.
   + Access the URL - http://<college-cloud-server-ip>/cc_registration
...
** Starting Lab containers
    Step 1. Go to the platform http://open-edx.vlabs.ac.in
    Step 2. Select the course/lab to be included in the workshop. Please
       make a note of the course/lab name.
    Step 3. Go to =http://<College-Cloud-Machine-IP>:3000= 
    Step 4. Sign into =OpenVZ web panel=. For credentials, please follow
       the [[./installation-manual.org::#passwords][passwords]]
    Step 5. Click on =localhost= located at left side of the panel.
    Step 6. Search for the lab name that has been selected in step 2.
    Step 7. Choose the selected lab by clicking on CTID (example.213) and
       look for =change state= button on the panel.
    Step 8. Select =start= and wait for about 1-2 minutes. 
    Step 9. Observe the change of color of the selected course/lab in green
       color and running state.

** Enabling Labs
   College Cloud Edition has 73 labs available across various
   ...
   + Step 1. Visit  http://vlabs.ac.in:18010/ to open Studio of College Cloud Edition.
   + Step 2. Login usin [[Passwords][admin credentials]]
   + Step 3. Select the course you want to enable for students to Enrol to.
   + Step 4. In Settings option, Click =Advanced Settings=. A list of parameters will be displayed 
   + Step 5. Look for =Invitation Only= option and set it to =false=
...
**** System Administrator
     If registered users forgot their password, System Administrator
     has to follow the below steps.
    :PROPERTIES:
    :CUSTOM_ID: admin
    :END:         
     - Visit  http://vlabs.ac.in/admin.
    ...
    - Go to =Authentication and Authorization= section to activate
      ...
      + Select =Users= 
      + Click on =user name=
      + Then change password of user 
	- Very small link there in first section of the page
...
** Workshop conducted by a Coordinator
   ...
   coordinator) who has account in =outreach.vlabs.ac.in= , only those
   can conduct the workshop using College Cloud Edition.
   1. [[https://github.com/vlabs-workshops/admin/blob/master/src/index.org][Plan the workshop]]
   2. Add workshop on [[http://outreach.vlabs.ac.in][Outreach Portal]].
      - visit =http://outreach.vlabs.ac.in=
      - Log on to outreach portal page.
      - Then, add workshop in outreach portal page.
   3. Make sure the [[./installation-manual.org::#requirements][requirements]] are met by the college.
...
** Analytics and Feedback : System Administrator
  ...
  :END:    
   There are two ways to get the analytics.
   1. Using CronJob
   2. By running the python script

*** Using CronJob 
    - Python scripts runs every day at 18:00 to get analytics of the
      workshop.
...
*** Run Syncthing
    ...
    :CUSTOM_ID: syncthing
    :END:      
    =Syncthing= is a tool to send collected analytics to VLEAD central
    repository.
    
    Internet is needed to College Cloud Edition to send the collected
    analytics to VLEAD central repo. To do this, follow the below
    steps.
 
    1. Disconnect from Wi-Fi router.(Unplug LAN cable that is
       connected to Wi-Fi router).
    2. Take Internet LAN cable and Connect this LAN cable to College
       Cloud Server.
    3. Log in as =vlabs= user
    4. Restart =network= service
       #+BEGIN_SRC 
       sudo service network restart
       #+END_SRC
    5. Fire the following command on terminal
       #+BEGIN_SRC 
       $syncthing
       #+END_SRC
** Analytics and Feedback : Workshop Coordinator
*** No internet in Collge Cloud Edition
   Please ask System Administrator for =<college-cloud-server-ip>=.
   1. Open a web browser on a machine connected to college-cloud/
      machine form which college-cloud was accessible.
   2. Access the URL - =http://<college-cloud-server-ip>/cc_registration=
      + Fill the details and click on =save= button to save the
        details
	- Note :: Please note that registration of college cloud
                  specified in the above step(2) is one time job only.
   3. Visti the URL - =http://<college-cloud-server-ip>/get_cc_mac_id=
      to know the =mac address= of the college cloud and also please note this
      for the future reference.
      - Note :: Obtained =Mac address= is useful when creating a offline workshop in
                outreach portal for reference pelase visit link [[http://community.vlabs.ac.in/docs/user-manuals/outreach-portal/#sec-3-3-1][Link]]
   4. Once the workshop is over , please ask System Administrator to
   ...
   6. Download the tar.gz file in the format
      =<College-name-Date>.tar.gz=
   7. Take downloaded file in pendrive and take a machine with
      internet connection.
   8. Upload the file in your google drive and share it with
      =engg@vlabs.ac.in=
*** Having internet in College Cloud Edition
    Please ask system administrator to run [[./installation-manual.org::#syncthing][syncthing]] in College
    Cloud Server.
...
* Proper shutdown of College Cloud Edition 
  College Cloud contains a Vagrant box which on direct shutdown can crash. 
  To avoid this problem, Sysadmin has to follow the steps given below -
  - Log in as =vlabs= user 
    #+BEGIN_EXAMPLE
    su - vlabs   
    #+END_EXAMPLE
