...
**** Syncthing
    ...
    The following steps should be used for sharing the logs using
    Syncthing :
 
    1. Disconnect from Wi-Fi router.(Unplug LAN cable that is
       connected to Wi-Fi router).
    2. Connect a Internet LAN cable to College Cloud Server.
    3. Login as [[./installation-manual.org::#passwords][vlabs user]] .
          + NOTE :: It is advised that the password is changed after the
              first login . For the steps for changing the password,
              follow the [[./installation-manual.org::#nagios_password][link]] .
...
** Analytics and Feedback : Workshop Coordinator
   ...
   + Use [[./installation-manual.org::#syncthing][Syncthing]] if internet connection is available in the college.
