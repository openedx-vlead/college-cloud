...
** Enhancing Performance
   1. If an *Intel i7 processor and 16GB RAM machine/system* is being used a maximum of 50-100 simultaneous users can access one lab at a time.
   2. It is adviced to have upto a maximum of 5 Labs running at any given time.
   3. The best choice would be start one lab at a time and have maximum of 10 labs running at any given instance.
...
**** Syncthing
    ...
    The following steps should be used for collecting and sharing the logs using
    Syncthing :
...
**** Manual Sharing
   ...
   +  Open a web browser on a machine connected to college-cloud
      server. 
   +  [[./installation-manual.org::#run_script][Run python script]] to collect analytics logs. 
   +  Access the URL -
      =http://<college-cloud-server-ip>/college-cloud-data/=
   6. Download the tar.gz file which would be in the format 
      =<College-name-Date>.tar.gz=
   7. Copy the downloaded file to a pendrive 
   8. Plugin your pendrive to a machine having internet connection. 
...
** Analytics and Feedback : Workshop Coordinator
   ...
   + [[./installation-manual.org::#registration][Register]] to College Cloud Edition (if not already done)
     Note :: 
      1. Visit the URL - =http://<college-cloud-server-ip>/get_cc_mac_id= to know the MAC
         address of the college cloud and save this for the future reference.   
      2. The MAC address is useful for computing usage for offline
         workshops from Outreach Portal.
   + Collect and share logs [[./installation-manual::#manual_sharing][manually]] if internet connection is not
     available in the college.
   + OR Collect and share logs automatically using [[./installation-manual.org::#syncthing][Syncthing]] if internet connection is available in the college.
...
* Proper shutdown of College Cloud Server
  ...
  - Log in as [[./installation-manual.org::#passwords][vlabs]] user
    #+BEGIN_EXAMPLE
    su - vlabs   
    #+END_EXAMPLE
  - Stop the vagrant box.
...
* Support  
  - Get help with Virtual Labs College Cloud Edition by 
    + Sending an email to =support@vlabs.ac.in= 
    + Posting queries on the IRC channel using =#VLEAD=.
    + Creating issues Issues on [[https://github.com/openedx-vlead/college-cloud/issues][Github repository of College Cloud]] 
