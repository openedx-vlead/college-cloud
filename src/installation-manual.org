...
* Requirements
  ...
  + College Cloud System Configuration (recommended) 
    ...
    #+END_EXAMPLE
  + WiFi Router : WiFi router which could serve 30-50 users.
...
** First time registration in College Cloud 
   We need your information to access and analyze logs. College has to
   register with College Cloud Edition by giving very basic
   information. Please follow the steps before starting to use College
   Cloud Edition.  (Please note that this is one time process and need
   not to be repeated after)
...
** Enabling labs on College Cloud Edition
   Text to be added 
...
*** Run Syncthing
    ...
    2. Change network interface file
       =/etc/sysconfig/network-scripts/ifcfg-br0= from
       #+BEGIN_EXAMPLE
       DEVICE=br0
       BOOTPROTO=static
       ONBOOT=yes
       ...
       ##DNS2=8.8.8.8
       #+END_EXAMPLE
    3. Disconnect from Wi-Fi router.(Unplug LAN cable that is
       connected to Wi-Fi router).
    4. Take Internet LAN cable and Connect this LAN cable to College
       Cloud Server.
    5. Restart =network= service
       #+BEGIN_SRC 
       service network restart
       #+END_SRC
    6. Log in as =vlabs= user
       #+BEGIN_EXAMPLE
       su - vlabs
       #+END_EXAMPLE
    7. Fire the following command on terminal
       #+BEGIN_SRC 
       $syncthing
       #+END_SRC
