...
* Collecting logs from College Cloud
  ...
  edX user data).
** Date wise Feedback dump
   A python program to fetch date wise feedback dump.   
#+NAME: fetch_feedbacks
#+BEGIN_SRC python
#!/usr/bin/python
...
** Fetch analytics and feedback
   ...
   A Shell script is given as utility which can get -
...
TAG=`sudo cat $CONFIG_FILE | jq ".[].college_details.college_name"|sed -e 's/\"//g'`-$DATE
#TAG=$COLLEGE_NAME-$DATE
LOCAL_WORKSHOP_DIR=$WORKSHOP_DIR/$WORKSHOP_FOLDER
EDX_AUTH_USER_DATA=edx-auth-user-data-$TAG-$DATE.sql
EDX_AUTH_REGISTRATION_DATA=edx-auth-registration-data-$TAG-$DATE.sql
...
full_dump()
{
    ...
    echo "`sudo vzctl exec $CTID_ANALYTICS elasticdump --input=http://localhost:9200/$INDEX_NAME --output=/root/vlabs-mapping-$TAG-$DATE.json --type=mapping`"
    echo "`sudo vzctl exec $CTID_ANALYTICS elasticdump --input=http://localhost:9200/$INDEX_NAME --output=/root/vlabs-data-$TAG-$DATE.json --type=data`"
    ...
    echo "`sudo vzctl exec $CTID_FEEDBACK mysqldump -u $MYSQL_USER -p$MYSQL_PASS $MYSQL_FEEDBACK_DB`" > $LOCAL_WORKSHOP_DIR/feedbackdump-$TAG-$DATE.sql
    echo "`date`: End Mysql dump for feedback " >> $LOCAL_WORKSHOP_DIR/exec-status-$DATE.txt
     ...
     # Details  
     # 1. SQLdump  gets copied in ~/college-cloud-data/workshop-analytics-$DATE folder
     # 2. filename will be feedbackdump-$TAG-$DATE.sql 
    ...
    echo "` cp $CONFIG_FILE  $LOCAL_WORKSHOP_DIR`"
...
datewise_dump()
{
     echo "-----DATE-WISE-DUMP------"
     echo "`date`: Start Mysql dump for feedback " >> $LOCAL_WORKSHOP_DIR/exec-status-$DATE.txt
     echo "`python ~/log_collection/feedback-dump.py`"
     echo "`mv feedback-dump-$DATE.json feedback-dump-TAG-$DATE.json `" 
     echo "`mv feedback-dump-$TAG-$DATE.json  $LOCAL_WORKSHOP_DIR/`" 
     echo "`date`: End Mysql dump for feedback " >> $LOCAL_WORKSHOP_DIR/exec-status-$DATE.txt
    
     echo "`date`: Start Vlabs analytics from ELK " >> $LOCAL_WORKSHOP_DIR/exec-status-$DATE.txt
     curl -XGET 'http://vlabs-analytics.vlabs.ac.in:9200/vlabs/_search?pretty&size=1000' -d '{ "query" : { "term" : { "DATE_OF_EXPERIMENT": $ELKDATE }}}' > vlabs-data-$TAG-$DATE.json
     echo "`mv vlabs-data-$TAG-$DATE.json  $LOCAL_WORKSHOP_DIR`" 
     ...
     #Copy the config.json file into $LOCAL_WORKSHOP_DIR. This file will be used in post workshop tasks at VLEAD. 
     #Using this file, VLEAD dev team will parse and pushes these logs into production.
...
#+END_SRC
**** Test case
 #+NAME: test_get_reg_form
 #+BEGIN_SRC python
class TestGetRegForm(TestCase):
    TESTING = True
    ...
    <div class="navbar navbar-default navbar-custom">
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" href="https://vlabs.ac.in"
...
<div class="container">
  ...
  <form action="/" method="POST">
	...
	<textarea class="form-control" name="clg_addr" placeholder="Enter college address" required></textarea>
      </div>
    </div>
    <div class="container">
	...
	<label><font color="red">*</font></label>
	<label>Pincode:</label>
	<input type="number" class="form-control" min="0" size="6" name="clg_pincode" placeholder="Enter pincode" required>
      </div>
    </div>
    <div class="container">
      <div class="form-group col-md-3">
	...
	+91<input type="number" class="form-control"  name="clg_contact_no" placeholder="Enter Mobile Number">
      </div>
    </div>
    <div class="container">
        <div class="form-group col-md-5">
...
<<wsgi_conf>>
#+END_SRC   
#+BEGIN_SRC shell :tangle log_collection/fetch_analytics.sh :eval no :noweb yes
<<fetch_analytics>>  
#+END_SRC
#+BEGIN_SRC python :tangle log_collection/feedback-dump.py :eval no :noweb yes
<<fetch_feedbacks>>  
#+END_SRC
