...
#+END_SRC
* Post Process script
  - This script will accept =<name>.tar.gz= file as command line argument
    and compute the =usage-analytics=, =feedback-analytics= and then
    dumps them to vlead's central elastic search database
...
def post_data_to_elastic_db(record, record_type, mac_id):
...
if __name__ == "__main__":
    ...
    try:
        feedback_file = glob.glob(pwd+"/feedbackdump*.json")
        sed_command = '''sed -ie 's/\"gateway_ip": "10.100.1.7\"/\"gateway_ip": "%s\"/g' %s''' % (mac_id, feedback_file[0])
        replace_ip_to_mac_id = os.system(sed_command)
	with open(feedback_file[0]) as data_file:    
	    feedbacks = json.load(data_file)
	if(post_data_to_elastic_db(feedbacks, "feedback", str(config['mac_id']))):
	    print ""
    ...
    try:
	with open(analytics_file[0]) as data_file:    
	    analytics_dump = json.load(data_file)
	if(post_data_to_elastic_db(analytics_dump, "analytics", str(config['mac_id']))):
	    print ""
	    #print "analytics : %s is added" % (analytics)
	else:
	    print "Error"
    except Exception as e:
	print str(e)
