...
* Implementation of College Cloud 
  - [[../../requirements/index.org::#portable_hard_drive][Requirement]] satisfied in this implementation 
  :PROPERTIES:
  :CUSTOM_ID: implementation_of_cc
  :END: 
  - Implementation is done based on the [[../../design/index.org::#cc_design][design diagram]]
...
*** Perform the below steps on host machine .
    *Minimum 1 Mbps download speed is needed. Need stable internet and
    network proxies if there are any.*
...
*** Once you have Vagrant box ready apply Comprehensive Theme
...
** Update Cluster's public DNS entries
   + Add these entries in
     =/var/named/{cluster}.virtual-labs.ac.in.forward= and
     =/var/named/{cluster}.vlabs.ac.in.forward=
     #+BEGIN_EXAMPLE
     open-edx         IN    A     10.2.57.65
     #+END_EXAMPLE
     - Replace =10.2.57.62= in the beginning of the file
       from
       #+BEGIN_EXAMPLE
       NS    IN    10.2.57.62
       #+END_EXAMPLE
...
* Collecting logs from College Cloud   
  - [[../../requirements/index.org::#req2][Requirement]] satisfied in this implementation
   :PROPERTIES:
   :CUSTOM_ID: log_collection
   :END: 
...
edx_auth_user_data = "edx-auth-user-data-" + tag_name + "-" + date_for_usage + ".sql"
edx_auth_registration_data = "edx-auth-registration-data-" + tag_name + "-" + date_for_usage + ".sql"
...
** Import libraries 
    Following libraries are used in the program. 
    + requests :: to make HTTP request call and response handling
    + os :: to fire system commands 
    + datetime :: to fetch date and time
    + json :: to operate on json files and data
    + logging :: to log events 
    + tarfile :: to work on tarfiles compression
    + variables :: a user defined custom file to have all variables
                   defined and used.
#+NAME: import_get_logs
#+BEGIN_SRC python
...
** set_dir_strucutre(<directory strucutre in string format>)
   + Function creates the directory structure requried to save logs collected. 
   + A common directory strucutre is ensured using this function.
   + Logs a DEBUG message "Directory structure already exists" in
     case structure exists.
   + Creates a strucutre if not found.

#+NAME: set_dir_structure
#+BEGIN_SRC python
...
** get_feedback(<url_string>, <filename in which the dump is to be saved>)
   + This function talks to the feedback service and dumps the reponse records inside the file.
   + Param 1 = url of feedback service to fetch records
   + Param 2 = name of the file in which the dump is to be saved.
...
** get_usage(<url_string>, <filename in which the dump is to be saved>)
   + This function will communicate with elasticsearch database and
     returns the usage records for specific date.
   + Param 1 :: url of elasticsearch server.
   + Param 2 :: Query to be passed as payload.
   + Param 2 :: the file in which the obtained dump is to be saved.
...
** get_user_logs(<vagrant_dir_path>, <script directory path>)
   + This function talks to Open edX database sitting inside the vagrant box using 
   + It fetches the records of user profiles and saves it in .sql file 
   + Param 1 :: the path of vagrant box directory to change the
                current directory to vagrant box directory.
   + Param 2 :: the path of script directory to change location back
                to the home of script.
...
** date_wise_dump()
   + This function will get all logs date wise, i.e the date on which
     the script is executed.
   + Parameters :: Does not expect any parameter
   + It call all other supporting functions required to collect user
     logs, feedback logs, and usage logs.
...
def date_wise_dump():
	...
	get_user_logs(vagrant_dir, script_dir,workshop_folder)
...
** full_dump()
   + This function will fetch all records (usages, feedbacks, and user logs)
   + Parameters :: Does not expect any paramters
   + Must be called when you need complete dump set from the day
     college cloud was installed.
...
def full_dump():
	...
	os.system("sudo vzctl exec " + ctid_analytics + " elasticdump --input=http://localhost:9200/vlabs --output=/root/vlabs-analytics-full.json --type=data")
	os.system("sudo cp /vz/private/" + ctid_analytics + "/root/vlabs-mapping-full.json " + full_dump_location)
	os.system("sudo cp /vz/private/" + ctid_analytics + "/root/vlabs-analytics-full.json " + full_dump_location)
        ...
        get_user_logs(vagrant_dir, script_dir,full_dump_location)
	make_tarfile(full_dumpfile, full_dump_location)

#+END_SRC

** make_tarfile(<output filename>, <source directory>)
   + This function will take the directory and using the tarball
     library of python converts and compress into .tar.gz format. 
   + Param 1 :: Expects the output file name of compressed files
   + Param 2 :: The directory path for all the files which are to be
                compressed.
#+NAME: make_tarfile
#+BEGIN_SRC python
...
* College Cloud registration
  - [[../../requirements/index.org::#req4][Requirement]] satisfied in this implementation
  - This form is required for collecting logs ( analytics, feedback
    and user data).
  - This form is a web form, should be able to access from a browser.
  - Workshop coordinator, need to fill college information in that
    form.  Once the Workshop coordinator submits form, college details
    will be saved in =/var/www/html/config.json= file.

  -  This config.json is being used by [[./index.org::#log_collection][log collection script]]
  :PROPERTIES:
...
#+BEGIN_EXAMPLE
python post_process.py <college_cloud_dump_file>.tar.gz
#+END_EXAMPLE
...
def post_data_to_elastic_db(record, record_type, mac_addr):
	...
	ELASTIC_DB_URL = "%s/dump_usage_to_elastic_db" % (FEEDBACK_URL)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print config['college_details']['mac_addr']
        
	if(post_data_to_elastic_db(config['college_details'], "config" , "")):
	    print "config : %s is added" % (config)
	    mac_addr = str(config['college_details']['mac_addr'])
	else:
	    print status_code
    ...
    try:
        ...
        print feedbacks
	if(post_data_to_elastic_db(feedbacks, "feedback", str(config['college_details']['mac_addr']))):	    
	...
	if(post_data_to_elastic_db(analytics_dump, "analytics", str(config['college_details']['mac_addr']))):	    
