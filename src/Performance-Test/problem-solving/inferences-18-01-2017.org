...
* Test Procedure and details 
  ...
  + Sample command 
    #+BEGIN_SRC 
    ab -c 10 -n 500 http://ps-iiith.vlabs.ac.in/
    #+END_SRC
  + The above command throws 500 total requests to the URL http://ps-iiith.vlabs.ac.in/
...
* Understanding the sample results -
  + Command used 
    #+BEGIN_SRC 
    ab -c 10 -n 1000 http://ps-iiith.vlabs.ac.in/
    #+END_SRC
...
* Inference
  ...
  #+BEGIN_SRC 
  When 10 requests are sent to http://ps-iiith.vlabs.ac.in/ at a time instance t1, it took 10.863 ms to complete 10 requests.
  Total 1500 rquests were sent in burst of 10. On an average each request takes 10.863 ms.
  #+END_SRC
** Assumtions 
  + Its quite likely that web caching at reverse proxy might have
    resulted into faster response.
  
  + Based on the above output, it can be assured that 100 people can
